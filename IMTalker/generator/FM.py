...
class FMGenerator(nn.Module):
    ...
    def sample(
        ...
        # Pre-generated shared noise, indexed by absolute frame position. The
        # live servers build this from --noise_seed under --shared_noise and put
        # it in data["noise_init"], but this class used to ignore the key
        # entirely (only deploy/8999_ab/FM_8999.py honoured it), so the
        # production avatar path silently drew its flow-matching noise from the
        # unseeded global RNG on every chunk -- making the motion, not just the
        # speech, different on every run.
        noise_init = data.get("noise_init")
        if isinstance(noise_init, torch.Tensor):
            noise_init = noise_init.to(device=device, dtype=torch.float32)
            if noise_init.ndim == 2:
                noise_init = noise_init.unsqueeze(0)
            if noise_init.shape[0] == 1 and B > 1:
                noise_init = noise_init.expand(B, -1, -1)
        else:
            noise_init = None
        ...
        for chunk_idx in range(num_chunks):
            if noise_init is not None:
                ...
                x0 = noise_init[:, start_idx:end_idx, :].to(device=device, dtype=torch.float32)
                if x0.shape[1] < self.num_frames_for_clip:
                    # Ran off the end of the pre-generated buffer: pad the
                    # remainder rather than failing. Raise --noise_max_frames if
                    # this happens routinely in a long session.
                    pad_len = self.num_frames_for_clip - x0.shape[1]
                    x0 = torch.cat(
                        [x0, torch.randn(B, pad_len, self.opt.dim_w, device=device)], dim=1
                    )
                elif x0.shape[1] > self.num_frames_for_clip:
                    x0 = x0[:, : self.num_frames_for_clip, :]
            elif self.opt.fix_noise_seed:
