...
- `stability/` - run-to-run reproducibility: the seed and runtime-contract
  controls, a fingerprint tool for diffing a good pod against a bad one, and
  the pinning workflow. Read this first if the same question ever gets a good
  answer on one pod and a wrong one on the next.
